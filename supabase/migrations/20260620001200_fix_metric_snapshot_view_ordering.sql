-- Fix: Prefer ok snapshots over missing_input in metric snapshot view
--
-- The original view selected snapshots using ORDER BY as_of_date DESC,
-- which caused newer missing_input snapshots to hide older valid ok snapshots.
--
-- This fix adds a CASE expression to prefer computation_status = 'ok'
-- before considering as_of_date. This ensures valid metrics are visible
-- even when newer missing_input snapshots exist.
--
-- All historical snapshots are preserved. No data is deleted.
