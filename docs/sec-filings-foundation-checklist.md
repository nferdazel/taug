...
- `[done]` detect amendment filings
- `[done]` create new `filing_versions` for amendments
- `[done]` connect supersession chain
- `[done]` emit `restatement_events` or equivalent audit event
...
- local companyfacts-parser validation on `2026-06-19` confirmed live `0000320193` parsing writes `reporting_periods`, `financial_statements`, and `financial_statement_items` rows from curated `10-K` / `10-Q` core facts; after hardening the replay path with bulk active-version lookup, preloaded period/statement caches, and batched item inserts, a repeat run finished in `10.89s` with `created_reporting_periods=0`, `replayed_reporting_periods=112`, `created_statements=0`, `replayed_statements=242`, `created_items=0`, and `replayed_items=1078`
- local statement-restatement validation on `2026-06-19` confirmed the synthetic amendment path links current and prior `financial_statements`, updates supersession status on both rows, emits one `restatement_event`, and emits one `financial_statement_restatement_linked` audit event; the current narrow live dataset did not contain any linked `10-K/A` or `10-Q/A` filing versions, so live statement-level restatement proof is still pending suitable source data
- local serving-view validation on `2026-06-19` confirmed `company_research_summary_v`, `company_latest_statement_facts_v`, and `filing_timeline_v` query live `AAPL` data successfully, including summary freshness, latest statement fact snapshot, and filing lineage timeline rows
